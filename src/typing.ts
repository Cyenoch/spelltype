...
import { prefixLength } from './format';
...
export class TypingController {
  ...
  private readonly handleInput = (event: Event): void => {
    const composing =
      this.composing || (event instanceof InputEvent && event.isComposing === true);
    if (composing) {
  ...
  private commitValue(rawNext: string): void {
    ...
    let next = rawNext;
    const points = [...next];
    if (points.length > MAX_INPUT_CODE_POINTS) {
      next = points.slice(0, MAX_INPUT_CODE_POINTS).join('');
    ...
    this.lastValue = next;
    this.accountEdit(previous, next);
  ...
  private accountEdit(previous: string, next: string): void {
    ...
    if (inserted <= 0) return; // deletion only: no new attempt
    const target = [...this.target];
    this.attempts += inserted;
    for (let index = head; index < head + inserted; index += 1) {
      if (after[index] !== target[index]) this.errors += 1;
    }
  }
