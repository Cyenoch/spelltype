...
以游戏为载体，练习打字速度、准确率与键盘熟练度。通过亲手输入英文咒文参与 2–4 人实时对战，支持快速匹配、排队练习和好友私人房。微信是唯一登录方式；账号、会话、对局结果和维护状态保存在数据库中。
...
- 人工管理员：微信登录后，持久化 `admin` 角色可访问 `/admin`，查看总览、用户、对局及咒文书；维护操作位于 `/admin/maintenance`。
- CI / 脚本：可选的维护专用令牌，只授权 `/api/ops/maintenance`，不授权账号管理或容器操作。
- 宿主机部署：使用现有 Docker 权限，应用容器不挂载 Docker socket。

后台列表支持服务端搜索、分页和关联详情跳转；用户详情支持永久封禁或按小时、天指定封禁时长，提交前确认账号与时长。管理员账号不可被封禁。封禁作用于该账号的全部会话，阻止对局、匹配与房间访问，并断开在线连接；登录后显示封禁状态，限时封禁按服务端时间自动到期。每次 API 请求重新检查管理员角色；后台响应不缓存，不返回微信身份、会话凭据或连接令牌。

历史对局按对局 ID 查询，不会混入复用房间的新对局。历史记录未保留的开始时间、结束原因和咒文快照显示为缺失；同主题的当前咒文书不代表历史版本。

个人战绩仅展示玩家成绩，不展示输入策略诊断。管理员可在对局结算记录中展开「触及规则」查看规则与次数；旧记录和未知版本不推断缺失的规则明细。
